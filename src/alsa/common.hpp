...
/// ALSA backend — internal header.
/// Not included by any public API header.
///
/// Used as a fallback when PulseAudio is unavailable (linux.cmake).
/// Requires libasound (pkg-config: alsa).
/// A dedicated fill thread calls MixerData::mixSamples() and writes
/// each block via snd_pcm_writei(), recovering from underruns automatically.
...
namespace systems::leal::campello_audio::alsa_backend {
...
// ---------------------------------------------------------------------------
// ALSAData — all ALSA state for one AudioEngine instance.
// ---------------------------------------------------------------------------

struct ALSAData {
    /// Opened PCM device — nullptr until init() succeeds.
    snd_pcm_t*        handle       = nullptr;

    /// Fill thread — calls mixSamples() + snd_pcm_writei() in a loop.
