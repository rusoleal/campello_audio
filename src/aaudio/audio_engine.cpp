/// @file audio_engine.cpp (AAudio backend)
/// @brief AudioEngine implementation for Android (API 26+).
///
/// ### Device setup
///   1. Create an AAudioStreamBuilder.
///   2. Request float32, desired sample rate / channel count,
///      low-latency shared-mode stream.
///   3. Register dataCallback → MixerData::mixSamples().
///   4. Register errorCallback → sets deviceLost on disconnection.
///   5. Open the stream; query actual sample rate & channel count back
///      (AAudio may negotiate different values).
///   6. Start the stream.
///
/// ### Thread model
///   AAudio owns the audio thread internally — there is no explicit
///   std::thread here.  dataCallback() is invoked on that thread.
...
using namespace systems::leal::campello_audio::aaudio;
...
static inline AAudioData* data(void* native) {
    return static_cast<AAudioData*>(native);
}
...
// ---------------------------------------------------------------------------
// AAudio callbacks — audio thread
// ---------------------------------------------------------------------------

aaudio_data_callback_result_t aaudio::dataCallback(AAudioStream* /*stream*/,
                                                    void*         userData,
                                                    void*         audioData,
                                                    int32_t       numFrames) {
    auto* d = static_cast<AAudioData*>(userData);
    d->mixer.mixSamples(static_cast<float*>(audioData),
                        static_cast<uint32_t>(numFrames));
    return AAUDIO_CALLBACK_RESULT_CONTINUE;
}

void aaudio::errorCallback(AAudioStream* /*stream*/,
                           void*         userData,
                           aaudio_result_t error) {
    if (error == AAUDIO_ERROR_DISCONNECTED) {
        static_cast<AAudioData*>(userData)->deviceLost.store(
...
bool AudioEngine::init(const AudioEngineDescriptor& descriptor) {
    ...
    AAudioStreamBuilder_setSampleRate(builder,
        static_cast<int32_t>(descriptor.sampleRate));
    AAudioStreamBuilder_setChannelCount(builder,
        static_cast<int32_t>(descriptor.channels));
    AAudioStreamBuilder_setPerformanceMode(builder,
        AAUDIO_PERFORMANCE_MODE_LOW_LATENCY);
    AAudioStreamBuilder_setSharingMode(builder,
        AAUDIO_SHARING_MODE_SHARED);
    AAudioStreamBuilder_setDataCallback(builder,
        aaudio::dataCallback, d);
    AAudioStreamBuilder_setErrorCallback(builder,
        aaudio::errorCallback, d);

    const aaudio_result_t openResult =
        AAudioStreamBuilder_openStream(builder, &d->stream);
    AAudioStreamBuilder_delete(builder);

    if (openResult != AAUDIO_OK || !d->stream) {
        delete d; return false;
    }

    // ---- Read back negotiated format ----
    // AAudio may have chosen a different sample rate or channel count.
    const int32_t actualRate = AAudioStream_getSampleRate(d->stream);
    const int32_t actualCh   = AAudioStream_getChannelCount(d->stream);
    if (actualRate > 0)
        d->mixer.config.sampleRate = static_cast<uint32_t>(actualRate);
    if (actualCh > 0)
        d->mixer.config.channels   = static_cast<uint32_t>(actualCh);

    // ---- Start ----
    if (AAudioStream_requestStart(d->stream) != AAUDIO_OK) {
        AAudioStream_close(d->stream);
        delete d; return false;
    }
...
void AudioEngine::deinit() {
    ...
    if (d->stream) {
        AAudioStream_requestStop(d->stream);
        AAudioStream_close(d->stream);
        d->stream = nullptr;
    }
